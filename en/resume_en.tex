...
Research interest focused on \textbf{Linux kernel security}, \textbf{software security}, \textbf{AI for security}. My research were published at top-tier security conferences including USENIX Security, CCS, S\&P.
...
\begin{itemize}
  \item \textbf{Conducted the first systematic fuzzing-based audit of the Linux kernel network stack's remote attack surface. Proposed the Fuzzer-in-the-Middle architecture, discovering 15 new remotely triggerable kernel vulnerabilities.}
...
\begin{itemize}
  \item \textbf{Proposed \emph{intra-cluster protocol poisoning} as a novel kernel attack surface for distributed file systems, and built a fuzzer that interposes on inter-node traffic and mutates protocol messages in flight, guided by static-analysis-based contamination-scope quantification and protocol-state-synchronized progressive exploration.}
  \item Discovered XX kernel vulnerabilities (heap overflows, use-after-free, null-pointer dereferences) across five distributed file systems (CephFS, SMB, NFS, 9P, etc.) running the latest Linux kernel, all triggerable by poisoned messages alone.
\end{itemize}
...
\resumeentry{CVE-2025-38089, CVE-2025-38051, CVE-2025-40210, CVE-2025-40212, CVE-2026-23220 \enspace|\enspace Linux Kernel Remote-Triggerable Vulnerabilities}{2025 - 2026}

\begin{itemize}
  \item Discovered five remote-triggerable vulnerabilities in the Linux kernel, spanning the KSMBD, NFS, and SUNRPC subsystems.
